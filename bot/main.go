...
type RecievedMessage struct {
	...
	Styledname string // don't care
	RoomId     string `json:"roomid"`
	Avatar     string // don't care
...
var songRequestFormat = regexp.MustCompile("^!sr.*youtube\\.com.*v=([\\w-]+)")
...
func handleMessages(messagesToSend chan<- SendMessage, messagesRecieved <-chan RecievedMessage) {
	for msg := range messagesRecieved {
		if msg.RoomId == SETTINGS.StreamerName && msg.From.Name != SETTINGS.BotName && msg.Type == ACTION_MESSAGE {
			...
			} else if SETTINGS.SongRequestEnabled && songRequestFormat.MatchString(msg.Message) {
				matches := songRequestFormat.FindStringSubmatch(msg.Message)
				link := fmt.Sprintf("https://youtube.com/watch?v=%s", matches[1])
				cmd := exec.Command("vlc", "--started-from-file", "--playlist-enqueue", link)
				err := cmd.Start()
				if err != nil {
					panic(err)
				}
			}
		} else if msg.From.Name != SETTINGS.BotName {
			fmt.Println("Ignoring unknown message:")
			fmt.Println(msg)
		}
	}
}
